...
class ProximalPolicyOptimization:
    def __init__(self, config, shared_network, extra):
        ...
        self.actor_opt = config.actor_optimizer_fn(
            shared_network.actor.parameters())
        self.critic_opt = config.critic_optimizer_fn(
            shared_network.critic.parameters())
    ...
    def episode(self, deterministic=False):
        config = self.config
        self.state_normalizer.offline_stats.load(
            self.shared_state_normalizer.offline_stats)
        self.reward_normalizer.offline_stats.load(
            self.shared_reward_normalizer.offline_stats)
        ...
        while not replay.full():
            ...
            for i in range(config.rollout_length):
                ...
                action = self.policy.sample(mean.data.cpu().numpy().flatten(
                ), std.data.cpu().numpy().flatten(), deterministic)
                ...
                state = self.state_normalizer(state)
                    ...
                    config.max_episode_length and episode_length > config.max_episode_length))
        ...
        self.shared_state_normalizer.offline_stats.merge(
            self.state_normalizer.online_stats)
        self.state_normalizer.online_stats.zero()

        self.shared_reward_normalizer.offline_stats.merge(
            self.reward_normalizer.online_stats)
        self.reward_normalizer.online_stats.zero()

        for _ in np.arange(self.config.optimize_epochs):
            self.worker_network.load_state_dict(
            ...
            probs_old = actor_net.log_density(
                actions, mean_old, log_std_old, std_old)
            ...
            obj_clipped = ratio.clamp(
                1.0 - self.config.ppo_ratio_clip, 1.0 + self.config.ppo_ratio_clip) * advantages
            ...
            nn.utils.clip_grad_norm(
                self.worker_network.parameters(), config.gradient_clip)
