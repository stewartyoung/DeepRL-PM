...
class DQNAgent:
    ...
    def episode(self, deterministic=False):
        ...
        steps = 0
        while True:
            value = self.learning_network.predict(
                np.stack([self.task.normalize_state(state)]), True).flatten()
            if deterministic:
            ...
            done = (done or (self.config.max_episode_length and steps >
            ...
            if not deterministic and self.total_steps > self.config.exploration_steps:
                ...
                if self.config.hybrid_reward:
                    ...
                    terminals = self.learning_network.to_torch_variable(
                    ...
                    q_action = []
                    actions = self.learning_network.to_torch_variable(
                        actions, 'int64').unsqueeze(1)
                    for q_ in q:
                    ...
                    q_next = self.target_network.predict(
                        next_states, False).detach()
                    if self.config.double_q:
                        _, best_actions = self.learning_network.predict(
                            next_states).detach().max(1)
                        q_next = q_next.gather(
            ...
            if not deterministic and self.total_steps % self.config.target_network_update_freq == 0:
                self.target_network.load_state_dict(
                    self.learning_network.state_dict())
            if not deterministic and self.total_steps > self.config.exploration_steps:
