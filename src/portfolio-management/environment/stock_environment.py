...
Based on Jiang et al [2017], https://arxiv.org/pdf/1706.10059.pdf
"""
from utils.plotNotebook import LivePlotNotebook
import logging
from utils.data import normalize, random_shift, scale_to_start, softmax
from utils.eps import eps
import numpy as np
import pandas as pd
...
class DataProvider(object):
    """
    Provides data for each new episode in the portfolio environment.
    """

    def __init__(self, df, steps=252, scale=True, scale_extra_cols=True, augment=0.00, window_length=50, random_reset=True):
        """
        Arguments:
            history_df: (num_stocks, timestep, 5) open, high, low, close, extra_feature
            abbreviations: a list of stock names for the desired portfolio
            steps: the total number of steps to simulate
            window_length: observation window (less than 50)
            start_date: string e.g. '2012-08-13'. Default is None and random
        """
        self.steps = steps + 1
    ...
    def _step(self):
        ...
        # (eq.1) prices
        # print("data_window[:, -1, 0] ->", data_window[:, -1, 0])
        # print("data_window[:, -2, 0] ->", data_window[:, -2, 0])

        # y1 is most recent days return (close_t/close_{t-1})
        ...
        if self.scale_extra_cols:
            # normalize each time step of extra feature by that timesteps mean and std. dev. (no lookahead bias)
            # row wise (each time step) mean and standard deviation
            mean = np.mean(data_window[:, :, nb_pc:], axis=0)
            means = np.array([mean]*data_window.shape[0])
            std = np.std(data_window[:, :, nb_pc:], axis=0)
            stds = np.array([std]*data_window.shape[0])

            data_window[:, :, nb_pc:] -= means
            data_window[:, :, nb_pc:] /= stds

        # get most recent days extra feature
        extra_feature = data_window[:, -1, nb_pc:].flatten()
        extra_feature = np.concatenate(
            [[0.0], extra_feature])  # add 0 as first extra feature
        self.step += 1
        history = data_window
        # print("history:\n", history)
        # print("y1:\n", y1)
    ...
    def reset(self):
        ...
        if self.random_reset:
            ...
            # print("low:", self.window_length + 1)
            # print("high:", self._data.shape[1] - self.steps - 2)
        else:
...
class PortfolioSim(object):
    ...
    def _step(self, w1, y1):
        ...
        y1 - price relative vector also called return
            e.g. [1.0, 0.9, 1.1]
        Numbered equations are from https://arxiv.org/abs/1706.10059 and Stewart Young final report
        """
        w0 = self.w0
        p0 = self.p0
        ...
        # Jiang (eq10) log rate of return
        r1 = np.log((p1 + eps) / (p0 + eps))
        # Jiang (eq22) immediate reward is log rate of return scaled by episode length
        reward = r1 / self.steps

        # remember weights and portfolio value for next step
        self.w0 = w1
        self.p0 = p1
...
class PortfolioEnv(gym.Env):
    ...
    def __init__(self,
                 ...
                 # as of may 2020 many online brokers offer zero commission on stocks, so I used 0.001
                 trading_cost=0.001,
                 time_cost=0.00,
                 window_length=50,
                 augment=0.00,
        ...
        # remember this information for saving plots with rigourous names
    ...
    def plot_notebook(self, close=False):
        ...
        if not self._plot:
            ...
            self._plot = LivePlotNotebook(
                log_dir="plots/", title='prices', labels=all_assets + ["Portfolio"], ylabel='value', colors=colors,
                filename=self.filename, timestamp=self.timestamp, proportion=self.proportion, repeat=self.repeat)
        x = df_info.index
        ...
        if not self._plot2:
            ...
            self._plot2 = LivePlotNotebook(
                log_dir="plots/", labels=all_assets, title='weights', ylabel='weight',
                timestamp=self.timestamp, proportion=self.proportion, repeat=self.repeat)
        ...
        if not self._plot3:
            ...
            self._plot3 = LivePlotNotebook(
                log_dir="plots/", labels=['cost'], title='costs', ylabel='cost',
                timestamp=self.timestamp, proportion=self.proportion, repeat=self.repeat)
